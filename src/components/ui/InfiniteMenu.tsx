...
function makeBuffer(gl: WebGL2RenderingContext, sizeOrData: ArrayBufferView, usage: number) {
      ...
      {activeItem && (
        <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-4 z-10">
          ...
          <div className="flex justify-between items-center">
            <div className={cn(
              "px-3 py-1 rounded-full bg-black/80 backdrop-blur-md border border-emerald-500/40 text-emerald-400 font-mono text-[11px] font-bold tracking-wider transition-opacity duration-300",
              isMoving ? "opacity-40" : "opacity-100"
            )}>
              {activeItem.badge || 'ACTIVE BREAKTHROUGH'}
            </div>
            <div className="text-[10px] font-mono text-slate-400 bg-black/70 px-2 py-0.5 rounded-md border border-white/10">
          ...
          <div className={cn(
            "bg-black/85 backdrop-blur-md p-4 rounded-2xl border border-white/20 text-white space-y-1 transition-all duration-300 shadow-2xl",
            isMoving ? "opacity-40 translate-y-2" : "opacity-100 translate-y-0"
          )}>
            <div className="text-lg font-black font-sans text-white leading-tight">
              {activeItem.title}
            </div>
